...
def main(ts: int = 121, batch_size: int = 1, sigma_v: float = 0.1):
    ...
    df_train = pd.read_csv(
        data_file_train, skiprows=1, delimiter=",", header=None, usecols=cols
    )
    df_val = pd.read_csv(
        data_file_val, skiprows=1, delimiter=",", header=None, usecols=cols
    )
    df_test = pd.read_csv(
        data_file_test, skiprows=1, delimiter=",", header=None, usecols=cols
    )
    df_train_time = pd.read_csv(
        data_time_train, skiprows=1, delimiter=",", header=None, usecols=cols
    )
    df_val_time = pd.read_csv(
        data_time_val, skiprows=1, delimiter=",", header=None, usecols=cols
    )
    df_test_time = pd.read_csv(
        data_time_test, skiprows=1, delimiter=",", header=None, usecols=cols
    )
    ...
    # net.load(f"saved_results/hq_g_seq_{input_seq_len}_multi.bin")
    net.load(f"saved_results/hq_g_seq_{input_seq_len}_5layer.bin")
    # Test
    mu_preds = []
    var_preds = []
    std_preds = []
    ...
    for itera in range(num_iter):
        ...
        for ts in ts_batch:
            ts_data[ts] = test_dtl_dict[ts].dataset["value"]
        ...
        for t in range(num_sample_inter):
            ...
            # var_x = np.nan_to_num(var_x, nan=0.0)
