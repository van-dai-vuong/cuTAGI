...
from pytagi.nn import LSTM, EvenExp, Linear, OutputUpdater, Sequential


def main(
    num_epochs: int = 50, batch_size: int = 16, sigma_v: float = 5, ts: int = 8
):
    ...
    for epoch in pbar:
        ...
        sigma_v = exponential_scheduler(
            curr_v=sigma_v, min_v=1e-10, decaying_factor=0.9, curr_iter=epoch
        )
        ...
        # print(f"Epoch #{epoch}. sigma_v = {sigma_v:0.5f}")
        # if epoch > 5:
        #     var_y = 0*var_y
        var_y = 0 * var_y
        ...
        for itera in range(num_iter):
            ...
            for ts in ts_batch:
                ...
                num_sample_inter = np.maximum(
                    len(ts_data[ts][1]), num_sample_inter
                )

            # x, _ = prep_x_y(0, ts_batch, ts_data, input_size)
            ...
            for t in range(num_sample_inter):
                ...
                # _x, y = prep_x_y(t, ts_batch, ts_data, input_size)
        ...
        if mse < mse_optim and epoch_optim < epoch:
            mse_optim = mse
            epoch_optim = epoch
            net.save(f"saved_results/hq_g_seq_{input_seq_len}_3layer_agvi.bin")
