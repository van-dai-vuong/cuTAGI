...
def main(num_epochs: int = 30, batch_size: int = 1, sigma_v: float = 1):
    ...
    hybrid = LSTM_SSM(
        ...
        baseline = 'trend',             # 'level', 'trend', 'acceleration'
    ...
    for epoch in pbar:
        ...
        mse_val = metric.mse(mu_preds, y_test)
        log_lik_val = metric.log_likelihood(
            prediction=mu_preds, observation=y_test, std=std_preds
        )

        # mse_val = metric.mse(mu_preds_unnorm_test, obs_test_unnorm)
        # log_lik_val = metric.log_likelihood(
        #     prediction=mu_preds_unnorm_test, observation=obs_test_unnorm, std=std_preds_unnorm_test
        # )
        ...
        plt.plot(idx_test,obs_test_unnorm, color='r',label=r"data")
        ...
        filename = f'saved_results/HQ_hybrid_epoch_#{epoch}.png'
