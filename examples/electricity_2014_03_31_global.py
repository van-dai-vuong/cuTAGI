...
def main(num_epochs: int = 50, batch_size: int = 16, sigma_v: float = 2):
    ...
    for ts in ts_idx:
        ...
        train_dtl_ = TimeSeriesDataloader(
        ...
        test_dtl_ = TimeSeriesDataloader(
        ...
        if ts == 0:
            ...
            # x_combined = np.concatenate((train_dtl.dataset["value"][0], train_dtl_.dataset["value"][0]), axis=0)
            # y_combined = np.concatenate((train_dtl.dataset["value"][1], train_dtl_.dataset["value"][1]), axis=0)
            # time_combined =  np.concatenate((train_dtl.dataset["date_time"], train_dtl_.dataset["date_time"]))
            # train_dtl.dataset["value"] = (x_combined, y_combined)
            # train_dtl.dataset["date_time"] = time_combined

            train_dtl = concatTsSample(train_dtl, train_dtl_)
            val_dtl   = concatTsSample(val_dtl, val_dtl_)
            test_dtl  = concatTsSample(test_dtl, test_dtl_)
    ...
    for epoch in pbar:
        ...
        pbar.set_description(
            f"Ts #{ts+1}/{nb_ts} | Epoch {epoch + 1}/{num_epochs}| mse: {sum(mses)/len(mses):>7.2f}",
            refresh=True,
        )
