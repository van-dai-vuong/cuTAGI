...
void lstm_cat_activations_and_prev_states(std::vector<float> &vec_a,
...
void lstm_update_hidden_state_posterior(std::vector<float> &mu_h_prior,
...
void lstm_update_cell_state_posterior(
...
LSTM::LSTM(size_t input_size, size_t output_size, bool last_timestep,
...
std::string LSTM::get_layer_name() const { return "LSTM"; }

LayerType LSTM::get_layer_type() const { return LayerType::LSTM; }

int LSTM::get_input_size() { return this->input_size * this->seq_len; }
    ...
    for (int t = 0; t < seq_len; t++) {
        ...
        parallel_for(end_chunk, this->num_threads, [&](int s, int e) {
            lstm_fwd_mean_var(this->mu_w, this->var_w, this->mu_b, this->var_b,
                              lstm_states.mu_ha, lstm_states.var_ha, s, e, ni_c,
                              no, batch_size, seq_len, t, this->bias,
...
void LSTM::backward(BaseDeltaStates &input_delta_states,
    ...
    if (seq_len == 1) {
        ...
        parallel_for(total_prior, this->num_threads, [&](int s, int e) {
            lstm_update_hidden_state_posterior(
        ...
        parallel_for(total_prior, this->num_threads, [&](int s, int e) {
            lstm_update_cell_state_posterior(
