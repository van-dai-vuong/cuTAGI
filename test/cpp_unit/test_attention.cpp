...
TEST(AttentionForward, BasicForwardPass) {
    ...
    std::vector<float> input_data(batch_size * seq_len * embed_dim);
    std::vector<float> input_var(batch_size * seq_len * embed_dim);
    std::vector<float> input_jcb(batch_size * seq_len * embed_dim, 1.0f);
    ...
    output_states.mu_a.resize(batch_size * seq_len * embed_dim);
    output_states.var_a.resize(batch_size * seq_len * embed_dim);
    output_states.jcb.resize(batch_size * seq_len * embed_dim);
    ...
    int expected_att_score_size = batch_size * num_heads * seq_len * seq_len;
        ...
        for (int j = 0; j < num_heads; j++) {
            for (int k = 0; k < seq_len; k++) {
                ...
                for (int l = 0; l < seq_len; l++) {
                    int idx = i * num_heads * seq_len * seq_len +
                              j * seq_len * seq_len + k * seq_len + l;
...
TEST(AttentionBackward, BasicBackwardPass) {
    ...
    input_delta_states.delta_mu.resize(batch_size * seq_len * embed_dim, 1.0f);
    input_delta_states.delta_var.resize(batch_size * seq_len * embed_dim, 0.1f);
    input_delta_states.block_size = batch_size;
    input_delta_states.actual_size = embed_dim * seq_len;
